...
func bucketExists(clients *clientmap.ClientMap, b *bucket.Bucket) (bool, string, error) {
	...
	clients.Each(func(region string, _ bool, client *s3.Client) {
		go func(client *s3.Client, region string) {
			res, err := checkBucketRegion(client, region, b)
			if err != nil {
				e <- err
				return
			}
...
// checkBucketRegion queries a single region for the bucket and classifies the
// response into an existence result, or an error for the caller to surface.
func checkBucketRegion(client *s3.Client, region string, b *bucket.Bucket) (bucketCheckResult, error) {
	...
	regionErr := probeBucketRegion(client, b)
	if regionErr == nil {
...
// probeBucketRegion issues the provider-appropriate request used to determine
// whether the bucket lives in the region served by client.
