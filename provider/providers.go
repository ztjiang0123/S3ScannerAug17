...
// checkBucketExists runs check (an existence probe such as bucketExists or bucketExists301) and records its
// outcome on b. It centralizes the mapping of the (exists, region, err) triplet onto the bucket's
// Exists/Region fields so every provider's BucketExists can share it.
func checkBucketExists(b *bucket.Bucket, check func() (bool, string, error)) (*bucket.Bucket, error) {
	exists, region, err := check()
	if err != nil {
		return b, err
	}
	if exists {
...
func bucketExists(clients *clientmap.ClientMap, b *bucket.Bucket) (bool, string, error) {
	...
	clients.Each(func(region string, _ bool, client *s3.Client) {
		go func(client *s3.Client, region string) {
			...
			regionErr := checkBucketRegion(client, b)
			if regionErr == nil {
			...
			if res, ok := classifyRegionError(regionErr, region, logFields); ok {
				results <- res
			} else {
				e <- unwrapRegionError(regionErr, logFields)
			}
		}(client, region)
	})
...
// checkBucketRegion probes whether the bucket exists by asking for its region, returning any error encountered.
func checkBucketRegion(client *s3.Client, b *bucket.Bucket) error {
...
// classifyRegionError maps a known region-check error to a bucketCheckResult. The second return value is false when
// the error is unexpected and should be surfaced to the caller instead.
func classifyRegionError(regionErr error, region string, logFields log.Fields) (bucketCheckResult, bool) {
	...
	bucketNotFound := errors.As(regionErr, &bnf) || errors.As(regionErr, &nsb)
	accessDenied := errors.As(regionErr, &re) && re.HTTPStatusCode() == 403

	switch {
	case bucketNotFound:
		log.WithFields(logFields).Debugf("BucketNotFound")
		return bucketCheckResult{region: region, exists: false}, true
	case accessDenied:
		log.WithFields(logFields).Debugf("AccessDenied")
		return bucketCheckResult{region: region, exists: true}, true
	default:
		return bucketCheckResult{}, false
	}
}

// unwrapRegionError logs an unexpected region-check error and returns the error to surface to the caller. If the error
// is a ResponseError, only the unwrapped error (e.g. "Method Not Allowed") is returned.
func unwrapRegionError(regionErr error, logFields log.Fields) error {
	var re *awshttp.ResponseError
	err := regionErr
	if errors.As(regionErr, &re) {
		err = re.Unwrap()
	}
